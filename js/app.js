...
async function loadTranslations() {
  try {
    const response = await fetch("data/lang.json?v=3.7.0");
...
async function loadProjectsFromJson() {
  try {
    ...
    if (localProjects) {
      ...
      const response = await fetch("data/projects.json?v=3.7.0");
...
async function loadSearchData() {
  try {
    const [projectsRes, blogRes] = await Promise.all([
      fetch("data/projects.json?v=3.7.0"),
      fetch("data/blog.json?v=3.7.0")
    ]);
